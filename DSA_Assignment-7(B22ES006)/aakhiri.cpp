...
public:
    ...
    map <int,vector <string>> parecls_till_time_tick;
    int max_parcel_capacity;
    int min_freight_cars_to_move;
    int time_tick;
    ...
    std::vector<Parcel*> get_bookings(int time_tick, const std::string& vertex) const {
    std::vector<Parcel*> result;
    for (const auto& parcel_pair : parcels) {
        const Parcel* parcel = parcel_pair.second;
        if (parcel->time_tick == time_tick && parcel->current_location == vertex) {
            result.push_back(const_cast<Parcel*>(parcel)); // Adding const_cast to maintain compatibility with the original design
        }
    }
    ...
    // Compare the old_state and new_state maps
    return old_state == new_state;
}


    bool all_parcels_delivered() const {
    return std::all_of(parcels.begin(), parcels.end(),
                       [](const auto& parcel_pair) {
                           return parcel_pair.second->delivered;
                       });
}
    ...
    for (const auto& [id, parcel] : parcels) {
        if (parcel->time_tick == t) {
            result.push_back(id + " " + parcel->current_location + " " + std::to_string(parcel->delivered));
        }
    }
